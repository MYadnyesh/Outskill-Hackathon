...
export async function generateJSON({ prompt, schema, temperature = 0.6, maxOutputTokens = 1024 }) {
  ...
  for (let i = modelCursor; i < MODELS.length; i++) {
    ...
    // Last model in the list gets whatever is left — there's nothing to save it for.
    const isLast = i === MODELS.length - 1;
    const attemptMs = isLast ? remaining : Math.min(remaining, MAX_ATTEMPT_MS);

    try {
      const result = await callModel({ model: MODELS[i], apiKey, body, schema, timeoutMs: attemptMs });
      if (i !== modelCursor) {
...
async function callModel({ model, apiKey, body, schema, timeoutMs }) {
  ...
  try {
    ...
    if (err?.name === 'AbortError') {
      ...
      throw new GeminiError('Gemini took too long to respond.', err, { canFallback: true });
    }
