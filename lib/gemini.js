...
// Ordered by OBSERVED SPEED AND AVAILABILITY, not by nominal capability.
//
// The -lite models lead. Repeated probing of the live API shows the full
// flash models are both the first to hit the 20/day free-tier cap and the
// worst affected under load — gemini-3.5-flash and gemini-3.7-flash have each
// been measured taking 48-57 SECONDS to answer a two-character prompt, while
// the -lite models answered the same prompt in under a second.
//
// Since the 20s timeout is a budget shared across the whole walk, one model
// that slow doesn't just fail — it starves every model behind it. Leading with
// -lite trades a little headroom on quality for an app that actually responds;
// their output was checked against real pages across all three modes and reads
// well. The full flash models stay in the list and still get used whenever
// they're healthy and the -lite entries are exhausted.
