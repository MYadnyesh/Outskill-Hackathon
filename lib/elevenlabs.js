...
export async function composeMusic({ compositionPlan, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  // buildCompositionPlan returns ONE OF TWO shapes, paired to the model:
  //   music_v2+ -> { chunks: [...] }
  //   music_v1  -> { positive_global_styles, ..., sections: [...] }
  // Checking only for `sections` rejected every plan built for the default
  // model, so audio silently degraded to the simulated player 100% of the
  // time. Accept either shape.
  const planParts = compositionPlan?.chunks?.length || compositionPlan?.sections?.length;
  if (!planParts) {
    throw new ElevenLabsError('composeMusic requires a composition plan with chunks or sections.');
  }
  ...
  // is whatever the parts add up to. Same two shapes as the guard above —
  // reading .sections unconditionally threw a TypeError on a chunk plan.
  const parts = compositionPlan.chunks || compositionPlan.sections || [];
  const plannedMs = parts.reduce((n, s) => n + (s.duration_ms || 0), 0);
